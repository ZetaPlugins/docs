...
You can use MiniMessage tags anywhere a format string is accepted, including `share-message.format`, `item-display.format`, `hover-text.layout`, `hover-text.enchantment-format`, `hover-text.stack-format`, and attribute formats.
...
```yml
<attributes:attack_damage> <attributes:attack_speed> <sprite:gui/statistics/item_crafted> <gray><durability_current></gray>
  ```
